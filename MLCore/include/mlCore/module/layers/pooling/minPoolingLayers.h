...
namespace MLCore::NN {
	/// <summary>
	/// A 1D min-pooling layer module that applies a sliding-window minimum over an input tensor.
	/// </summary>
	/// <typeparam name="T">The numeric data type used by the layer's tensors (for example float or double).</typeparam>
	template <typename T>
	class MinPool1DLayer : public Module<T> {
	public:
		/// <summary>
		/// Constructs a MinPool1DLayer with the specified filter length, stride, padding, dilation, and ceil mode.
		/// </summary>
		...
		MinPool1DLayer(size_t filterLength, size_t stride = 0, size_t padding = 0, size_t dilation = 1, bool ceilMode = false);

		/// <summary>
		/// Performs the forward pass of a 1D min-pooling layer on the provided input tensor.
		/// </summary>
		...
		/// <returns>A tensor containing the result of applying 1D min pooling to the input using the layer's configured filter length, stride, padding, dilation, and ceil-mode settings.</returns>
	...
	/// <summary>
	/// A 2D min-pooling layer that performs spatial downsampling by taking the minimum value over sliding windows.
	/// </summary>
	/// <typeparam name="T">Element type of the input and output tensors (for example float or double).</typeparam>
	template <typename T>
	class MinPool2DLayer : public Module<T> {
	public:
		/// <summary>
		/// Constructs a 2D min-pooling layer configured with the specified kernel size, strides, padding, dilation, and ceil mode.
		/// </summary>
		...
		MinPool2DLayer(size_t filterHeight, size_t filterWidth, size_t strideH = 0, size_t strideW = 0,
					   size_t paddingH = 0, size_t paddingW = 0, size_t dilationH = 1, size_t dilationW = 1, bool ceilMode = false);

		/// <summary>
		/// Performs the forward pass of a 2D min-pooling layer, applying min pooling to the input tensor using the layer's configured filter size, strides, padding, dilation, and ceil mode.
		/// </summary>
		...
		/// <returns>A TensorCore::Tensor containing the result of the 2D min-pooling operation.</returns>
		virtual TensorCore::Tensor<T> Forward(const TensorCore::Tensor<T>& input) override;
	...
	/// <summary>
	/// Layer that performs 3D min pooling over an input tensor.
	/// </summary>
	/// <typeparam name="T">The numeric data type of the elements stored in the input and output tensors (e.g., float, double).</typeparam>
	template <typename T>
	class MinPool3DLayer : public Module<T> {
	public:
		/// <summary>
		/// Constructs a MinPool3DLayer initialized with the given 3D pooling kernel sizes, strides, paddings, dilations, and ceil mode.
		/// </summary>
		...
		MinPool3DLayer(size_t filterDepth, size_t filterHeight, size_t filterWidth,
		...
		/// <summary>
		/// Performs the forward pass of a 3D min-pooling layer on the given input tensor using the layer's configured filter size, strides, padding, dilation, and ceil-mode.
		/// </summary>
		...
		/// <returns>A tensor containing the result of applying 3D min pooling to the input. The output shape is determined by the layer's filter dimensions, strides, paddings, dilations, and ceil-mode.</returns>
		virtual TensorCore::Tensor<T> Forward(const TensorCore::Tensor<T>& input) override;
