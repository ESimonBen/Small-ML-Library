...
namespace MLCore::NN {
	/// <summary>
	/// A templated 1D convolutional layer module that applies a convolution over the length (temporal) dimension and stores kernel and bias parameters.
	/// </summary>
	/// <typeparam name="T">Element type used by tensors and parameters (e.g., float or double).</typeparam>
	template <typename T>
	...
	public:
		...
		Conv1DLayer(size_t inChannels, size_t outChannels, size_t kernelLength, size_t stride = 1, size_t padding = 0, size_t dilation = 0, InitType kernelInit = InitType::XavierUniform, InitType biasInit = InitType::Zero);

		/// <summary>
		/// Applies this Conv1D layer to an input tensor, performing a 1D convolution using the layer's kernel, bias, stride, padding, and dilation settings.
		/// </summary>
	...
	protected:
		...
		/// <summary>
		/// Appends this Conv1DLayer's named constant parameters (kernel and bias) to the provided output vector, using an optional name prefix.
		/// </summary>
	...
	/// <summary>
	/// A 3D convolutional neural network layer that applies learnable kernels over depth, height, and width to produce output feature maps.
	/// </summary>
	/// <typeparam name="T">The numeric element type used for tensors and layer parameters (e.g., float, double).</typeparam>
	template <typename T>
	...
	public:
		...
		/// <param name="strideH">Vertical stride (height) for the convolution.</param>
		/// <param name="strideW">Horizontal stride (width) for the convolution.</param>
		/// <param name="paddingD">Depth padding applied to the input.</param>
		/// <param name="paddingH">Vertical padding applied to the input.</param>
		/// <param name="paddingW">Horizontal padding applied to the input.</param>
		/// <param name="dilationD">Depth dilation (spacing) of kernel elements.</param>
		/// <param name="dilationH">Vertical dilation (spacing) of kernel elements.</param>
		/// <param name="dilationW">Horizontal dilation (spacing) of kernel elements.</param>
		/// <param name="kernelInit">Initialization method (InitType) used to initialize the kernel tensor.</param>
		/// <param name="biasInit">Initialization method (InitType) used to initialize the bias tensor.</param>
		Conv3DLayer(size_t inChannels, size_t outChannels, size_t kernelDepth, size_t kernelHeight, size_t kernelWidth,
		...
		/// <summary>
		/// Applies this Conv3D layer to an input tensor, performing a 3D convolution using the layer's kernel, bias, stride, padding, and dilation settings.
		/// </summary>
		/// <typeparam name="T">The element type of the tensors (e.g., float or double) used for the input, kernel, bias, and output.</typeparam>
		/// <param name="input">The input tensor to convolve. Supplied by const reference and must have a layout and element type compatible with this layer.</param>
		/// <returns>A new TensorCore::Tensor<T> containing the result of the 2D convolution. The method is const and does not modify the layer's stored parameters.</returns>
	...
	protected:
		...
		/// <summary>
		/// Appends this Conv3DLayer's named constant parameters (kernel and bias) to the provided output vector, using an optional name prefix.
		/// </summary>
		/// <typeparam name="T">The numeric/tensor element type used by the layer (the type parameter for ConstNamedParameter).</typeparam>
		/// <param name="name">Optional prefix for parameter names. If empty, the parameter suffixes "kernel" and "bias" are used as-is; otherwise the prefix and suffix are joined with a '.' (e.g., "prefix.kernel").</param>
		/// <param name="out">Reference to a vector that will receive ConstNamedParameter<T> entries for the layer's kernel and bias; entries are appended to the vector.</param>
