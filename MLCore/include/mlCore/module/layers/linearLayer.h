...
namespace MLCore::NN {
	...
	public:
		...
		/// <returns>A new TensorCore::Tensor containing the result of the linear transformation (weight * input + bias). The tensor is allocated using the input tensor's allocator.</returns>
	...
	protected:
		...
		/// </summary>
		/// <typeparam name="T">The value type for the layer's parameters (the type stored in NamedParameter, e.g., a numeric or tensor element type).</typeparam>
		...
		/// <param name="out">Reference to an output vector that will be appended with NamedParameter entries referring to the layer's weight and bias.</param>
		...
		/// <param name="out">Output vector to which ConstNamedParameter entries are appended. Each entry contains the generated name and a reference to the corresponding member (m_Weight or m_Bias).</param>
