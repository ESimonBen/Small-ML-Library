...
namespace MLCore::Operations {
	...
	/// <returns>A new TensorCore::Tensor with dimensions swapped (rows ↔ columns) containing the transposed elements. If the input required gradients, the returned tensor will also require gradients and have its gradient function set appropriately.</returns>
	template <typename T>
	...
	/// <returns>A TensorCore::Tensor shaped as {1} whose single element is the dot product (sum of element-wise products). If either input required gradients, the returned tensor will require gradients and include an attached gradient function.</returns>
	template <typename T>
