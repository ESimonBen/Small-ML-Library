...
namespace MLCore::Optimizers {
	...
	public:
		Adam(std::vector<Parameter<T>>& params, T learningRate = static_cast<T>(.001), T weightDecay = static_cast<T>(0), T beta1 = static_cast<T>(.9),
	...
	private:
		...
		std::unordered_map<Parameter<T>*, TensorCore::Tensor<T>> m_FirstMoment;
		std::unordered_map<Parameter<T>*, TensorCore::Tensor<T>> m_SecondMoment;
	};

	template <typename T>
	class AdamW : public Optimizer<T> {
	public:
		AdamW(std::vector<Parameter<T>>& params, T learningRate = static_cast<T>(.001), T weightDecay = static_cast<T>(0), T beta1 = static_cast<T>(.9),
			T beta2 = static_cast<T>(.999), T epsilon = static_cast<T>(1e-8));
