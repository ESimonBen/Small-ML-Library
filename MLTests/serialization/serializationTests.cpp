...
TEST_SUITE("Serialization Tests") {
	TEST_CASE("Binary Archive Tests") {
		SUBCASE("Write and Read basic data to a file") {
			{
				std::filesystem::path path = "testFiles/test1.ckpt";
			...
			std::ifstream in{ "testFiles/test1.ckpt", std::ios::binary };
		...
		SUBCASE("Write and Read an array/string to a file") {
			{
				std::filesystem::path path = "testFiles/test2.ckpt";
				auto parentPath = path.parent_path();
			...
			std::ifstream in{ "testFiles/test2.ckpt", std::ios::binary };
		...
		SUBCASE("Write and Read a tensor to a file") {
			{
				std::filesystem::path path = "testFiles/test3.ckpt";
				auto parentPath = path.parent_path();
			...
			std::ifstream in{ "testFiles/test3.ckpt", std::ios::binary };
	...
	TEST_CASE("Checkpoint Tests") {
		SUBCASE("Save and Load model parameters") {
			...
			Checkpoint::Save(model, "testFiles/checkpointTest1.ckpt");
			...
			Checkpoint::Load(model2, "testFiles/checkpointTest1.ckpt");
		...
		SUBCASE("Save and Load named parameters") {
			...
			Checkpoint::Save(model, "testFiles/checkpointTest2.ckpt");
			...
			Checkpoint::Load(model2, "testFiles/checkpointTest2.ckpt");
		...
		SUBCASE("Save and Load optimizer") {
			...
			Checkpoint::Save(model, "testFiles/checkpointTest3.ckpt", &opt);
			...
			Checkpoint::Load(model2, "testFiles/checkpointTest3.ckpt", &opt2);
		...
		SUBCASE("Save and Load learning rate scheduler") {
			...
			Checkpoint::Save(model, "testFiles/checkpointTest4.ckpt", &opt, &scheduler);
			...
			Checkpoint::Load(model2, "testFiles/checkpointTest4.ckpt", &opt2, &scheduler2);
		...
		SUBCASE("Save and Load training state") {
			...
			Checkpoint::Save<float>(model, "testFiles/checkpointTest5.ckpt", nullptr, nullptr, &state);

			Trainer<float> trainer2{ model, opt, lossFn };

			auto state2 = trainer2.GetState();

			Checkpoint::Load<float>(model, "testFiles/checkpointTest5.ckpt", nullptr, nullptr, &state2);
